...
import { Award, ShieldCheck, Twitter, Download, ExternalLink, Calendar, GitPullRequest, ArrowRight, Loader2, Sparkles, ThumbsUp, Lightbulb } from "lucide-react";
...
export default function AcademyVerify() {
  ...
  // Fetch AI PR review for this certificate
  const { data: reviewData } = useQuery<{ success: boolean; review: PRReview | null }>({
    queryKey: ['academyReview', certId],
    queryFn: async () => {
      const res = await fetch(getApiUrl(`/api/academy/review/${certId}`));
...
Stay evergreen. 🌲💻`;
  ...
  return (
    <div className="min-h-screen bg-background overflow-x-hidden custom-scrollbar relative">
      ...
      <main className="w-full max-w-5xl mx-auto px-4 md:px-8 pt-24 pb-32 md:pb-12 space-y-12 relative z-10">
        ...
        {review && (
          <div className="animate-fade-up max-w-3xl mx-auto">
            <div className="rounded-2xl border border-primary/20 bg-card p-6 md:p-8 space-y-6">
              <div className="flex items-center justify-between gap-4">
                <div>
                  <h2 className="text-lg font-bold text-foreground flex items-center gap-2">
                    <Sparkles className="w-5 h-5 text-primary" /> Capstone PR Review
                  </h2>
                  <p className="text-xs text-muted-foreground mt-1">
                    Generated by an AI maintainer review of your merged pull request.
                  </p>
                </div>
                <div className="flex flex-col items-center shrink-0">
                  <span className="text-4xl font-black font-mono text-primary">{review.score}<span className="text-base text-muted-foreground">/10</span></span>
                  <span className="text-[9px] uppercase tracking-wider text-muted-foreground">Grade</span>
                </div>
              </div>
              ...
              <div className="grid sm:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <ul className="space-y-1.5">
                    {review.strengths.length > 0 ? review.strengths.map((s, i) => (
                      <li key={i} className="text-xs text-muted-foreground flex gap-2">
                        <span className="text-primary shrink-0">•</span> {s}
                      </li>
                    )) : (
                      <li className="text-xs text-muted-foreground italic">No strengths captured.</li>
                    )}
                  </ul>
                </div>
                <div className="space-y-2">
                  ...
                  <ul className="space-y-1.5">
                    {review.improvements.length > 0 ? review.improvements.map((s, i) => (
                      <li key={i} className="text-xs text-muted-foreground flex gap-2">
                        <span className="text-amber-500 shrink-0">•</span> {s}
                      </li>
                    )) : (
                      <li className="text-xs text-muted-foreground italic">Nothing major to improve.</li>
                    )}
                  </ul>
                </div>
              </div>
              ...
              {review.prUrl && (
                <div className="pt-4 border-t border-border/60 flex items-center gap-2 text-xs text-muted-foreground">
                  ...
                  <a href={review.prUrl} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline truncate">
