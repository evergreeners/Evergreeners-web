...
async function main() {
    const users = await db.select({ email: schema.users.email, name: schema.users.name }).from(schema.users);
    const recipients = users.filter((u) => u.email && u.email.includes('@'));

    console.log(`Found ${users.length} users; ${recipients.length} with a usable email address.`);
    ...
    if (process.env.DRY_RUN === 'true') {
        ...
        recipients.forEach((u) => console.log(`  - ${u.email} (${u.name || 'no name'})`));
        return;
    }
